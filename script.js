...
// Your existing usernameCodes object (omitted for brevity, copy from original scripts.txt)
const usernameCodes = {
  A: {
...
// Your existing quizData object (omitted for brevity, copy from original scripts.txt)
const quizData = {
  "Use of English": [
    {
      question:
...
// Anti-inspection measures (unchanged)
...
async function initApp() {
  ...
  try {
    ...
    if (loggedInUser) {
      ...
      if (data.isSubscribed && data.subscriptionEnd > new Date().getTime()) {
        isSubscribed = true;
...
// Updated submitAccessCode function
...
function startQuiz() {
  ...
  try {
    ...
    const maxQuestions = isSubscribed
      ? Math.min(questionCount, availableQuestions)
      : Math.min(15, availableQuestions);
    ...
    if (questionCount > availableQuestions) {
      showMessage(
        'start-error',
        `Only ${availableQuestions} questions available for ${currentCourse}. Selecting ${availableQuestions} questions.`
      );
    ...
    score = 0;
    answered = [];
    selectedQuestions = isSubscribed
      ? shuffleArray([...quizData[currentCourse]]).slice(0, questionCount)
      : quizData[currentCourse].slice(0, maxQuestions);
